...
> ✅ **Largely implemented 2026-06-18** (commits `83d5ecd`, `bf8c927`). Resolved: malformed `</hé>`; Bootstrap 4 / jQuery / Font Awesome removal; descriptive `alt` text; carousel `aria-label`s; inline-style → `.card-work`; duplicate `.card-container`; `delete resizeTimeout`; `ms` use-before-declaration; cached 2D context; tab-hidden pause.
> ⚠️ **Reverted:** the `prefers-reduced-motion` gating (JS + CSS) was removed — it stopped the hero starfield entirely on systems that report `reduce` (verified). The starfield now always animates. **Still open (deferred — see §7 Phase C):** SRI hashes on the remaining CDN `<link>`s, full IIFE/module encapsulation of the starfield globals, and image format/responsive optimisation.
...
> ✅ **Partly implemented 2026-06-18:** removed the redundant CDN libraries (Bootstrap 4 / jQuery / Font Awesome); added `defer` to scripts and `loading="lazy"` / `decoding="async"` to below-the-fold images; cached the canvas 2D context; pause the starfield on tab-hidden. *(A `prefers-reduced-motion` guard was added then reverted — it froze the hero starfield on systems reporting `reduce`.)* **Still open:** image format/responsive optimisation (§5.1.2) and minify/compression (§5.2) — see §7 Phase C.
...
- Add a `prefers-reduced-motion` guard for the animations — *tried and reverted: it froze the hero starfield on systems reporting `reduce`. Left out by design so the starfield always animates.*
